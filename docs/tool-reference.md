...
- **Pagination metadata.** `total` is the `total_count` the contacts response carries alongside the collection itself, so it costs no extra request and a truncated read is visible without paging until an empty page comes back. The CRM plugin renders it: `app/views/contacts/index.api.rsb` wraps the array in `api_meta(:total_count => @contacts_count, :offset => @offset, :limit => @limit)` (confirmed on 4.4.5 Pro). `has_next` is derived from the total -- a further page exists when `offset + limit` is below it -- rather than from "the page came back full", so a collection whose size is an exact multiple of the page size is not reported as having one more page than it has. `list_redmine_issues` still infers `has_next` from a full page and can be wrong on that boundary. If a build does render no `total_count`, `total` stays `null` -- read that as "not reported", never as a number -- while `has_next` falls back to that same full-page inference rather than going `null`, because a `null` is falsy and a caller testing it would stop mid-collection. `count`, `limit`, `offset`, `has_previous` and `previous_offset` never depend on the total and are always exact. `limit` and `offset` are the window Redmine reports having applied, so `next_offset` lands on a real page boundary even if a build serves a smaller page than was asked for. A `search` narrows the returned page without narrowing the reported total, so with one in play `count` is what this call matched and `total` is the collection it searched.
