...
- **Pydantic-validation-error boundary middleware**: a new FastMCP middleware (`CleanValidationErrorMiddleware`) intercepts argument-validation `pydantic.ValidationError`s raised before the tool body runs and returns the project's standard envelope (`{"error", "hint", "code": "INVALID_ARGUMENTS", additional_errors?}`) as both text and `structured_content`. Previously, the raw Pydantic v2 dump (including `errors.pydantic.dev` URLs) leaked through to the MCP caller, which was verbose and not actionable inside an LLM loop ([#108](https://github.com/jztan/redmine-mcp-server/issues/108)).
