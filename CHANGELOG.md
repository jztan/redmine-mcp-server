...
### Added
- `REDMINE_MCP_MIRROR_REDMINE_AS_METADATA=true` opt-in for OAuth mode. When enabled, `/.well-known/oauth-authorization-server/mcp` mirrors Redmine's own RFC 8414 / OpenID Connect discovery metadata before applying MCP-specific `scopes_supported`. This lets Redmine deployments that advertise optional capabilities such as Dynamic Client Registration expose them to MCP clients without adding discovery fetches for stock Redmine deployments.
